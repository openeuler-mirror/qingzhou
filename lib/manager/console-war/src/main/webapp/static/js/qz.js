;!function (window, undefined) {
        ...
        this.init = function (dom, minFn, destroyFn) {
            if ($(dom, this.rootBox).length > 0) {
                var html = ""
                    ...
                    + "    </div>"
                    + "    <div class=\"tab-body\" style=\"height:100%;\"></div>"
                    + "</div>";
    ...
    window.qz = {
        ...
        /**
         * 为目标对象绑定事件及响应结果目标渲染对象(注意：请勿将实际业务相关的代码逻辑写入，尽量保持与具体业务操作隔离)
         * @param {String} triggerSelector 触发事件的元素选择器表达式
         * @param {String} targetSelector 响应结果的目标渲染元素
         * @param {bool} append （true 表示在目标渲染元素中追加响应结果 | false 表示覆盖目标渲染元素的内容。通常情况下请用 false）
         * @param {bool} resetData (列表页搜索条件是否需要重置搜索条件，用于搜索、分页、列表数据刷新等)
         * @param {Dom Objects} restrictedArea 限定区域元素，此参数若省去，则默认为 document.body (注：此参数的作用主要是限制选择响应结果的目标渲染元素，避免获取到多个目标元素导致渲染异常)
         * @param {json} callbacks 事件绑定及事件响应过程中不同阶段的可调用函数，目前支持：beforeFill、afterFill，后续如有更多需要，可以扩展支持更多函数。
         * @returns {void}
         */
        bindFill: function (triggerSelector, targetSelector, append, resetData, restrictedArea, callbacks) {
            ...
            $(triggerSelector + "[loaded!='true']", restrictedArea).attr("loaded", "true").click(function (e) {
                ...
                that.fill(url, data, $(targetSelector, restrictedArea), append, callbacks);
            });
        },
        /**
         * ajax 填充 html 片段
         ...
         */
        fill: function (url, data, container, append, callbacks) {
            if(callbacks == null || callbacks == undefined) {
                callbacks = {};
            }
            var that = this;
            ...
            $.ajax({
                ...
                success: function (html) {
                    ...
                    if(callbacks["beforeFill"] && (typeof callbacks["beforeFill"]) === "function") {
                        callbacks["beforeFill"].call(null, container);
                    }
                    if (append) {
                    ...
                    if(callbacks["afterFill"] && (typeof callbacks["afterFill"]) === "function") {
                        callbacks["afterFill"].call(null, container);
                    }
