...
+function ($) {
    ...
    Tree.prototype._setUpListeners = function () {
        ...
        $(this.element).on("click", this.options.trigger, function (event) {
            if ($(this).attr("action") !== undefined && $(this).parent().attr("class").indexOf("menu-open") < 0) {
                $.ajax({
                    type: "POST",
                    ...
                    success: function (data) {
                        if (data.msg !== "" && data.msg !== null && data.msg_level != "info") {
...
function handleError(e) {
    if (e.status === 0) {
        ...
        if (e.status === 200 && e.responseText.indexOf("<!DOCTYPE html>") >= 0 && e.responseText.indexOf("loginForm") > 0) {
            ...
            showMsg(e.status + ":" + e.responseText, "error");
        }
    }
};
