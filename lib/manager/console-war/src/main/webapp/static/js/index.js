...
function setOrReset(container) {
    ...
    $("aside.main-sidebar[loaded!='true']", document.body).each(function () {
        ...
        var self = this;
        qz.bindFill("aside.main-sidebar[bindingId='" + bindingId + "'] ul.sidebar-menu a[loaded!='true']", ".main-body[bindingId='" + bindingId + "']", false, true, $(this).parent(), {"beforeFill": function() {
...
function initializeManager(element, url) {
    ...
    bindTabEvent();
    qz.fill(url, {}, $(".content-box>ul>li").last(), false, {"afterFill" : function () {
                    ...
                    success: function (res, textStatus, xhr) {
                        ...
                        if (xhr.getResponseHeader("Content-Type") && xhr.getResponseHeader("Content-Type").includes("application/json")) {
                            if ((res.success === false || res.success === 'false') && res.msg) {
                                ...
                                let form_open=document.querySelector("a[href=\""+actionUrl+"\"]");
                                let tof=form_open.getAttribute("isSubFormSubmitOnOpen");
                                if(tof=="true") {
...
function openTab(dataId, dataUrl, tabTitle) {
    ...
    qzTab.addTab(dataId, tabTitle, "", {}, true, function (tabDom) {
        qz.fill(dataUrl, {}, tabDom, false, {"afterFill": function () {
