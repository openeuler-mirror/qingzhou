...
function bindEventForListPage() {
    ...
    $("section.main-body", document.body).each(function () {
        ...
        // 弹出表单
        $(domSelector + " a[action-type='sub_form']", restrictedArea).each(function () {
            var actionIdSelector = "";
            if ($(this).attr("action-id") !== undefined) {
                actionIdSelector = "[action-id='" + $(this).attr("action-id") + "']";
            }
            var selector = domSelector + " a[action-type='sub_form']" + actionIdSelector + (containSubTab ? ":not(div.tab-container a)" : "");
            bindingActions.sub_form(null, selector, restrictedArea);
        });
