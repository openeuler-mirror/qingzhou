...
function updateCombine(target) {
    ...
    if (val) {
        ...
        for (let value of values) {
            var html = "<li class='combine'><span> "
                + JSON.stringify(value)
                + " </span> <a class='icon icon-times' onClick='delCombine(event)'></a></li>";
            ulEl.append(html)
        }
...
function addCombineGroup(target, fieldName) {
    ...
    var html = "<form class='filterForm' >" + formHtml + "</form> <hr style='margin-top: 4px;'>";
...
function delCombine(e){
    ...
    if (values.length > 0) {
        ulEl.closest("div.form-control").children("input").val(JSON.stringify(values));
    } else {
        ulEl.prev("span").css("display", "block");
        ulEl.closest("div.form-control").children("input").val("");
    }
}

function addCombine(target, fieldName) {
