...
function updateCombine(target) {
    var ulEl = $(target).parent().find("ul:first");
    ulEl.children("li").remove();
    var val = $(target).val();
    if (val) {
        var values = JSON.parse(val);
        for (let value of values) {
            var html = "<li class='combine-item'><span> "
                + JSON.stringify(value)
                + " </span> <a class='icon icon-times combine-item-del' onClick='delCombine(event)'></a></li>";
            ulEl.append(html)
        }
        $(target).next("div").children("span").css("display", "none");
    } else {
        $(target).next("div").children("span").css("display", "block");
    }
}

function addCombineGroup(target, fieldName) {
    var html = $("#combine-group-" + fieldName).html();
    html = "<form class='filterForm'>" + html + "</form><hr style='margin-top: 4px;'>";
    $(target).parent().before(html);
}

function delCombineGroup(target) {
    var formEl = $(target).closest("form");
    formEl.next().remove();
    formEl.remove();
}

function delCombine(e) {
    e.stopPropagation();
    var liEl = $(e.target).parent();
    var ulEl = liEl.parent();
    liEl.remove();
    var values = [];
    ulEl.find("li>span").each(function () {
        values.push(JSON.parse($(this).text()));
    });
    if (values.length > 0) {
        ulEl.closest("div.combine").children("input").val(JSON.stringify(values));
    } else {
        ulEl.prev("span").css("display", "block");
        ulEl.closest("div.combine").children("input").val("");
    }
}

function openCombine(target, fieldName) {
    var values = $("input[name='" + fieldName + "']", target).val();
    if (values) {
        values = JSON.parse(values);
    } else {
        values = [{}];
    }
    var formHtml = $("#combine-group-" + fieldName).html();
    var html = "";
    for (let i = 0; i < values.length; i++) {
        html += "<form class='filterForm'>" + formHtml + "</form><hr style='margin-top: 4px;'>";
    }

    var id = "combine-form-" + fieldName;
    html = "<div style='margin-top: 10px;' id='" + id + "' >" + html
        + "<span class='input-group-btn' style='padding-left: 10px;'><a class='btn' onclick='addCombineGroup(this,\"" + fieldName + "\")' ><i class='icon icon-plus-sign'></i> " + getSetting('addBtnText') + "</a> </span>"
        + "</div>";
    ...
    openLayer({
        ...
        success: function (layero, index, that) {
            $(layero).children("div.layui-layer-content").css("overflow", "scroll");
            $("#" + id, layero).children("form").each(function (index) {
                updateFormData($(this), values[index], []);
            })
        },
        yes: function (index, layero, that) {
            var values = [];
            $("#" + id, layero).children("form").each(function () {
                var data = {};
            ...
            var $input = $("input[name='" + fieldName + "']", target);
            if (values.length > 0) {
        ...
        $(this).multipleSelect({
            ...
            onAfterCreate: function () {
                $("ul li.multiple", $($this).next(".ms-parent")).each(function () {
                    ...
                    var color = $(this).parent().parent().parent().siblings("select[multiple='multiple']").find("option[value=" + name.replace(/([ #;?%&,.+*~\':"!^$[\]()=>|@/])/g, '\\$1') + "]").css("color");
            ...
            onOpen: function () {
                var $dropdown = $($this).next(".ms-parent").find(".ms-drop");  // 获取下拉框
                var dropdownHeight = $dropdown.outerHeight();  // 获取下拉框高度
                var dropdownOffset = $($this).offset().top;  // 获取下拉框的顶部偏移量
                var windowHeight = $(window).height();  // 获取窗口高度
                var availableSpaceBelow = windowHeight - dropdownOffset - dropdownHeight;  // 下方可用空间
                var availableSpaceAbove = dropdownOffset - $(window).scrollTop();  // 上方可用空间

                // 如果下方空间不足，并且上方有足够的空间，则将下拉框调整为向上显示
                if (availableSpaceBelow < 0 && availableSpaceAbove > dropdownHeight) {
                    $dropdown.css({
                        top: 'auto',
                        bottom: '100%',  // 向上显示
                        marginBottom: '4px',  // 可选：调整距离
                        marginTop: '0'  // 可选：调整距离
                    });
                } else {
                    // 否则，默认向下显示
...
function setOrReset(container) {
    ...
    $("aside.main-sidebar[loaded!='true']", document.body).each(function () {
        ...
        var self = this;
        qz.bindFill("aside.main-sidebar[bindingId='" + bindingId + "'] ul.sidebar-menu a[loaded!='true']", ".main-body[bindingId='" + bindingId + "']", false, true, $(this).parent(), {
            "beforeFill": function () {
                var toRemove = $(self).parent().children("div[returnsBlock]");
                if ($(toRemove).length > 0) {
                    $(toRemove).children("section").appendTo($(self).parent());
                    $(self).next().remove();
                    toRemove.remove();
                }
            }
...
function initCombine() {
    $("div.combine").each(function () {
        updateCombine($("input", this));
    })
}
...
function bindEventForFormPage() {
    ...
    $(".form-btn a[action-type='" + getSetting("qrActionName") + "']").unbind("click").bind("click", function (e) {
        e.preventDefault();
        ...
        openLayer({
            ...
            yes: function (index) {
                var params = {};
                params[getSetting("otp")] = $.trim($("#randCode-OTP").val());
                $.ajax({
...
function groupMultiselectOptions(parentGroupDescriptions, groupedOptions, multiselectOptions) {
    const GROUP_SEPARATOR = getSetting("multiselectGroupSeparator");
    // 存储父组的键
    const parentGroups = new Set();

    // 处理父组关系
    multiselectOptions.forEach(entry => {
        const key = entry.name;

        // 查找是否有以当前key为前缀的子项，若有则标记当前key为父组
        multiselectOptions.forEach(subEntry => {
            const subKey = subEntry.name;
            if (subKey !== key && subKey.startsWith(key + GROUP_SEPARATOR)) {
                parentGroups.add(key);
                return; // 跳出内层循环，避免重复计算
            }
        });
    });
    ...
    multiselectOptions.forEach(entry => {
        ...
        if (parentGroups.has(value)) {
            parentGroupDescriptions.set(value, getI18n(entry.i18n));
        } else {
            // 提取组名并将选项存入对应的组
            const separatorIndex = value.lastIndexOf(GROUP_SEPARATOR);
            let groupName = '';
            if (separatorIndex !== -1) {
            ...
            if (!groupedOptions.has(groupName)) {
                groupedOptions.set(groupName, new Map());
            }
            groupedOptions.get(groupName).set(value, getI18n(entry.i18n));
        }
    });
}
    ...
    for (let option of options) {
        ...
        switch (type) {
            case "grouped_multiselect":
                ...
                //获取placeholder
                placeholder = $("select", formItem).attr("placeholder");
                //渲染原始html
                html += "<select echoGroup=\"+ echoGroup +\" name=\"" + option.field + "\" multiple=\"multiple\" style=\"width:100%;\"\n" +
                    "        placeholder=\"" + placeholder + "\">";
                const parentGroupDescriptions = new Map();  // Map 用来存储父组描述
                const groupedOptions = new Map();  // Map 用来存储组内的选项
                groupMultiselectOptions(parentGroupDescriptions, groupedOptions, option.options)
                parentGroupDescriptions.forEach(function (value, key, map) {
                    html += "<optgroup label=\"&nbsp;&nbsp;" + value + "\">";
                    let groupMap = groupedOptions.get(key);
                    groupMap.forEach(function (value, key, map) {
                        if (option.value === value) {
                            html += "<option value='" + key.replace(/"/g, '&quot;') + "' selected>" + value + "</option>";
                        } else {
                            html += "<option value='" + key.replace(/"/g, '&quot;') + "'>" + value + "</option>";
                        }
                    })
                    html += "</optgroup>";
                });
...
var bindingActions = {
        ...
        "export": function (dom, selector, restrictedArea) {
            $(selector + "[loaded!='true']", restrictedArea).attr("loaded", "true").bind("click", function (e) {
                e.preventDefault();
                var filterForm;
                if ($("form[name='filterForm']", getRestrictedArea(true)).length > 1) {
                    filterForm = $("form[name='filterForm']", getRestrictedArea(true))[1];
                } else {
                    filterForm = $("form[name='filterForm']", getRestrictedArea(true));
                }
                var data = {}
                if (filterForm) {
                ...
                for (const item of data) {
                    actionUrl += item.name + "=" + item.value + "&";
                }
        ...
        "sub_form": function (dom, selector, restrictedArea) {
            $(selector + "[loaded!='true']", restrictedArea).attr("loaded", "true").bind("click", function (e) {
                ...
                if ($(this).attr("href") !== "#" && $(this).attr("href").indexOf("javascript:") < 0) {
                    popupAction(
                        $(this).attr("action-id"),
                        $(this).closest("section.main-body"),
                        $(this).attr("sub_form_id"));
                }
                return false;
            });
        },
        ...
        "returns_link": function (dom, selector, restrictedArea) {
            $(selector + "[loaded!='true']", restrictedArea).attr("loaded", "true").bind("click", function (e) {
                e.preventDefault();
                var genId = qz.guid();
                var cycleDom = $("section.main-body:first", restrictedArea);
                $(cycleDom).after("<div id='" + genId + "' style='display:none;' returnsBlock='" + genId + "'></div>");
                $(cycleDom).appendTo("#" + genId);
                $("#" + genId).before($("#" + genId).html());
                var guid = qz.guid();
                var newDom = $("#" + genId).prev();
                $(newDom).attr("bindingId", guid);
                $(".bodyDiv:first", newDom).attr("bindingId", guid);
                //传递filterForm表单的参数
                var filterForm;
                if ($("form[name='filterForm']", getRestrictedArea(true)).length > 1) {
                    filterForm = $("form[name='filterForm']", getRestrictedArea(true))[1];
                ...
                data[getSetting("paramNameReturnsId")] = genId;
                //如果href中有了则剔除data中的
                const href = $(this).attr("href");
                data = data.filter(function (item) {
                    return !href.includes(item.name + "=");
                });
                qz.fill(href + (href.indexOf("?") > 0 ? ("&") : "?") + getSetting("paramNameReturnsId") + "=" + genId, data, newDom, false, null);
                return false;
            });
        }
    };
...
function bindEventForListPage() {
    ...
    $("section.main-body", document.body).each(function () {
        ...
        // 列表页表格单元格操作
        qz.bindFill(domSelector + " table a.dataid[action-type!='" + getSetting("actionTypeReturns") + "']" + (containSubTab ? ":not(div.tab-container a)" : ""), domSelector + ">.bodyDiv:first", false, false, restrictedArea, null);
        // 返回按钮
        qz.bindFill(domSelector + " a[action-type='" + getSetting('back') + "']", domSelector + ">.bodyDiv:first", false, false, restrictedArea, null);

        // 列表页表格顶部操作按钮
        $(domSelector + " div.qz-list-operate>.tools-group>a.btn:not([act-confirm]" + (containSubTab ? ":not(div.tab-container a)" : "") + ")", restrictedArea).each(function () {
            if ($(this).attr("id") === undefined) {
                $(this).attr("id", qz.guid());
            }
            var selector = "#" + $(this).attr("id");
            var actionTypeMethod = bindingActions[$(this).attr("action-type")];
            if (actionTypeMethod) {
                actionTypeMethod.call(null, this, selector, restrictedArea);
            } else {
                qz.bindFill(selector, domSelector + ">.bodyDiv:first", false, false, restrictedArea, null);
            }
        });
        // 列表页表格操作按钮
            ...
            if (actionTypeMethod) {
                ...
                if ($(this).attr("action-id") === getSetting("actionId_app_manage")) {// 集群实例点击[管理]，打开新 Tab 并切换
                    $(selector + "[loaded!='true']", restrictedArea).attr("loaded", "true").bind("click", function (e) {
                        ...
                        return initializeManager($(this), $(this).attr("href"), true);
                    });
                } else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.msg === undefined){
                    msg = data.data[fieldStr]
                }
                showMsg(msg, data.msg_level);
            },
            error: function (e) {
                handleError(e);
            }
...
function initializeManager(element, url, closeTab) {
    var tabHtml = "<li id=\"" + new Date().getTime() + "\" bind-id=\"" + element.attr("data-id") + "\">"
        ...
        + "</a>";
    if (closeTab === true || closeTab === 'true') {
        tabHtml += (
            "<label class=\"close\">"
    ...
    bindTabEvent();
    qz.fill(url, {}, $(".content-box>ul>li").last(), false, {
        "afterFill": function () {
...
function confirm_method(filterForm, url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success === "true" || data.success === true) {
                var searchBtn = $(".filter_search", getRestrictedArea(true));
...
function popupAction(actionId, restrictedArea, autoloadId) {
    var sub_form_action_div = $("div[sub_form_action_id='" + actionId + "']", restrictedArea);
    let html = sub_form_action_div.html();
    var autoload = sub_form_action_div.attr("sub_form_autoload");
    var autoClose = sub_form_action_div.attr("sub_form_autoclose");
    var baseUrl = sub_form_action_div.attr("sub_form_base_url");
    let questionMarkIndex = baseUrl.indexOf('?');
    var actionUrl = "";
    if (questionMarkIndex !== -1) {
        actionUrl = baseUrl.slice(0, questionMarkIndex) + "/" + sub_form_action_div.attr("sub_form_action_name") + "/" + autoloadId + baseUrl.slice(questionMarkIndex);
    } else {
        actionUrl = baseUrl + "/" + sub_form_action_div.attr("sub_form_action_name") + "/" + autoloadId;
    }
    var title = sub_form_action_div.attr("sub_form_action_title");
    ...
    openLayer({
        ...
        success: function (layero, index, that) {
            $(document.getElementById(formId)).on('submit', function (e) {
                e.preventDefault();
                var passwordFields = $("div[name='passwordFields']", this).text().trim().split(",");
                var encrypt = new JSEncrypt({"default_key_size": getSetting("encrypt_key_size")});
                encrypt.setPublicKey($("div[name='pubkey']", this).text().trim());
                let formData = $(this).serializeArray();
                formData = formData.map(item => {
                    if (passwordFields.includes(item.name)) {
                        item.value = encrypt.encryptLong2(item.value);
                    }
                    return item;
                });
                $.ajax({
                    ...
                    success: function (res, textStatus, xhr) {
                        ...
                        if (xhr.getResponseHeader("Content-Type") && xhr.getResponseHeader("Content-Type").includes("application/json")) {
                            ...
                            if (htmlRegex.test(res)) {
                                $popupForm.after('<hr style=\'margin-top: 4px;\'>' + res);
                            } else {
                                $popupForm.after('<hr style=\'margin-top: 4px;\'>' + '<pre>' + res + '</pre>');
                            }
                            layer.style(index, {height: '500px'});
                        }
                    },
            ...
            if (autoloadId) {
                ...
                let questionMarkIndex = baseUrl.indexOf('?');
                if (questionMarkIndex !== -1) {
                    requestUrl = baseUrl.slice(0, questionMarkIndex) + "/" + getSetting("showActionName") + "/" + autoloadId + baseUrl.slice(questionMarkIndex);
                } else {
                    requestUrl = baseUrl + "/" + getSetting("showActionName") + "/" + autoloadId;
                }
                $.ajax({
                    type: "GET",
                    url: requestUrl,
...
function resizeChart(chartObj, option) {
    ...
    if (chartObj.renderFlag === undefined || chartObj.renderFlag) {
        ...
        if (option.series.length == 0) {
            $('div[monitorTop="top"]').css('display', 'none');
        } else {
            chartObj.resize();
        }
    }
...
/**用于列表跳转后返回列表 */
function backToList(dom) {
    $("#" + $(dom).attr("linkId")).prev().remove();
    $("#" + $(dom).attr("linkId")).children().appendTo($("#" + $(dom).attr("linkId")).parent());
    $("#" + $(dom).attr("linkId")).remove();
}
