...
public class Controller implements Process {
    ...
    private CompositeProcess sequence;
    ...
    private class StartLocalApp implements Process {
        @Override
        public void run() {
            deployer.setLoaderPolicy(QingzhouSystemApp.class::getClassLoader);
            deployer.systemApp = FileUtil.newFile(moduleContext.getLibDir(), DeployerConstants.APP_MASTER);
            doStartApp(deployer.systemApp);

            deployer.setLoaderPolicy(moduleContext::getApiLoader);
