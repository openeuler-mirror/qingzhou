...
class DeployerImpl implements Deployer {
    ...
    private AppManagerImpl buildApp(File appDir, Properties deploymentProperties) throws Throwable {
        ...
        java.util.List<File> scanAppLibFiles = new ArrayList<>(); // 使用 List 而非 Set,是为保持 jar 加载顺序
        scanAppLibFiles.add(appDir); // 此处将应用的根目录增加到 classpath 中 防止类似 config/ application.properties 资源无法加载
        // 将“根目录”下的 jar 文件加入应用加载路径
        Arrays.stream(Objects.requireNonNull(appDir.listFiles())).filter(f -> f.getName().endsWith(".jar")).forEach(scanAppLibFiles::add);
        // 探测“根目录/lib” 下所有的 jar 文件加入应用加载路径
        appendClassPath(new File(appDir, "lib"), scanAppLibFiles);
        // SpringBoot 等应用实践会从 /config 加载资源
        scanAppLibFiles.add(new File(appDir, "config"));
                ...
                if (listFiles != null) {
                    for (File jarFile : listFiles) {
                        if (jarFile.getName().endsWith(".jar")) {
                            Properties properties = FileUtil.zipEntryToProperties(jarFile, DeployerConstants.QINGZHOU_PROPERTIES_FILE);
                            if (properties != null) {
                                result = properties;
                                break;
                            }
