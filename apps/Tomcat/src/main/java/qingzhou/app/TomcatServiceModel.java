...
public class TomcatServiceModel extends ModelBase implements AddModel {
    // todo: 应用（Tomcat）的启动和停止应该在 extends QingZhouApp 的子类中实现
    // todo：应用（Tomcat）的安装和删除是框架对应用包的操作，不应该在此Model里面实现
    // todo：此 Model 里面应编写 Tomcat 内部的模块管理相关逻辑，如 安全域、虚拟主机、连接器等等
