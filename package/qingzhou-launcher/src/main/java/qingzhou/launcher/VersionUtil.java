...
class VersionUtil {
    ...
    private File getHomeDir() throws IOException {
        ...
        String qzHome = System.getProperty("qingzhou.home");
        if (qzHome == null || qzHome.trim().isEmpty()) {
            qzHome = System.getenv("QINGZHOU_HOME");
        }
        if (qzHome == null || qzHome.trim().isEmpty()) {
            ...
            qzHome = new File(pre).getPath();
        }
        qingzhouHomeFile = new File(qzHome).getCanonicalFile();
