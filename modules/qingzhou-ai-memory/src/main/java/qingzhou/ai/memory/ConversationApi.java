...
/**
 * 会话治理 REST 端点（B-4）：会话列表 / 全量消息 / 改名 / 删除。
 * <p>
 * 端点放本模块而非 qingzhou-ai：治理接口 {@link ChatMemoryAdmin} 保持 bundle 私有（不导出包），
 * 同包引用零跨模块依赖；若放 qingzhou-ai 则必须导出接口并使其编译期+运行时硬依赖本模块，
 * 破坏"记忆实现可选部署、无记忆时聊天照常"的降级设计。
 * 实际路径由 http-server 按 bundle 名拼接前缀：/ai-memory/conversations[...]。
 * <p>
 * 协议：
 * <ul>
...
public class ConversationApi implements HttpHandler {
    ...
    private void listConversations(HttpResponse httpResponse, String userId) throws Exception {
        List<ConversationSummary> conversations = admin.listConversations(userId);
    ...
    private void listMessages(HttpResponse httpResponse, String userId, String conversationId) throws Exception {
        // 会话不存在或加载失败时接口契约返回空列表（见 ChatMemoryAdmin），前端据此以空会话呈现
        List<HistoryMessage> messages = admin.listMessages(userId, conversationId);
        Map<String, Object> body = new HashMap<>();
    ...
    private void renameConversation(HttpRequest httpRequest, HttpResponse httpResponse, String userId,
        ...
        // 空串/非字符串统一归一为 null（未命名），与存储层"null 表示未命名"的语义对齐
        String titleStr = title instanceof String && !((String) title).trim().isEmpty()
                ? ((String) title).trim() : null;
        if (!admin.renameConversation(userId, conversationId, titleStr)) {
            sendError(httpResponse, 404, "NOT_FOUND");
            return;
        }
    ...
    private void deleteConversation(HttpResponse httpResponse, String userId, String conversationId) throws Exception {
        if (!admin.deleteConversation(userId, conversationId)) {
            sendError(httpResponse, 404, "NOT_FOUND");
            return;
        }
        sendJson(httpResponse, 200, OK_BODY);
    ...
    /** 注册前缀（/ai-memory/conversations）之后的剩余路径；前缀定位失败（防御）返回 null */
