...
public class JVMConfig extends ModelBase implements Editable {
    ...
    @ModelField(nameI18n = {"初始堆内存", "en:Initial Heap Size"}, infoI18n = {"设置初始堆内存大小 (-Xms)。", "en:Set the initial heap size (-Xms)."})
    @FieldView(group = group_memory)
    public String Xms;

    @ModelField(nameI18n = {"最大堆内存", "en:Max Heap Size"}, infoI18n = {"设置最大堆内存大小 (-Xmx)。", "en:Set the max heap size (-Xmx)."})
    @FieldView(group = group_memory)
    public String Xmx;

    @ModelField(nameI18n = {"新生代大小", "en:Xmn Size"}, infoI18n = {"设置新生代大小 (-Xmn)。", "en:Set the new generation size (-Xmn)."})
    @FieldView(group = group_memory)
    public String Xmn;

    @ModelField(nameI18n = {"老年代比率", "en:New Ratio"}, infoI18n = {"设置新生代和老年代的大小比率，通俗地讲即老年代比新生代的倍数 (-XX:NewRatio)。", "en:Set the size ratio of the new generation and the old era, which is colloquially speaking, that is, the multiple of the old era to the new generation (-XX:NewRatio)."})
    @FieldView(group = group_memory, type = FieldType.number)
    @FieldValidation(numberMin = 1, numberMax = 100, effectiveWhen = "Xmn=")
    public Integer NewRatio;

    @ModelField(nameI18n = {"Eden 区比率", "en:Survivor Ratio"}, infoI18n = {"设置新生代中 Eden 区域和 Survivor 区域（From 幸存区或 To 幸存区）的比率 (-XX:SurvivorRatio)。", "en:Sets the ratio of the Eden zone to the Survivor zone (From Survivor Zone or To Survivor Zone) in the new generation (-XX:SurvivorRatio)."})
    @FieldView(group = group_memory, type = FieldType.number)
    @FieldValidation(numberMin = 1, numberMax = 100)
    public Integer SurvivorRatio;

    @ModelField(nameI18n = {"初始元空间", "en:MetaspaceSize"}, infoI18n = {"设置初始元空间的大小 (-XX:MetaspaceSize)。", "en:Set the initial Metaspace size (-XX:MetaspaceSize)."})
    @FieldView(group = group_memory)
    public String MetaspaceSize;

    @ModelField(nameI18n = {"最大元空间", "en:MaxMetaspaceSize"}, infoI18n = {"设置最大元空间的大小 (-XX:MaxMetaspaceSize)。", "en:Set the max metaspace size (-XX:MaxMetaspaceSize)."})
    @FieldView(group = group_memory)
    public String MaxMetaspaceSize;

    @ModelField(nameI18n = {"线程栈大小", "en:Stack Size"}, infoI18n = {"设置线程栈大小的最大值 (-Xss)。", "en:Set the max thread stack size (-Xss)."})
    @FieldView(group = group_memory)
    public String Xss;

    @ModelField(nameI18n = {"堆外内存", "en:MaxDirectMemorySize"}, infoI18n = {"设置最大堆外内存，当 Direct ByteBuffer 分配的堆外内存达到该大小后，即触发 Full GC (-XX:MaxDirectMemorySize)。", "en:Set the maximum out-of-heap memory. When the out-of-heap memory allocated by Direct ByteBuffer reaches this size, Full GC is triggered (-XX:MaxDirectMemorySize)."})
    @FieldView(group = group_memory)
    public String MaxDirectMemorySize;

    @ModelField(nameI18n = {"垃圾回收器", "en:GarbageCollector"}, infoI18n = {"设置 JVM 回收内存使用的垃圾回收器。", "en:Set up the garbage collector used by the JVM to reclaim memory."})
    @FieldView(group = group_memory, type = FieldType.radio)
    public String useGC;

    @ModelField(nameI18n = {"并行收集线程数", "en:ParallelGCThreads"}, infoI18n = {"设置并行收集器收集时使用的CPU数，即并行收集线程数 (-XX:ParallelGCThreads)。", "en:Set the number of CPUs used by the parallel collector collection, that is, the number of parallel collection threads (-XX:ParallelGCThreads)."})
    @FieldView(group = group_gc, type = FieldType.number)
    @FieldValidation(effectiveWhen = "useGC=UseParallelGC|useGC=UseConcMarkSweepGC", numberMin = 1, numberMax = 100)
    public Integer ParallelGCThreads;
    @ModelField(nameI18n = {"最大暂停时间", "en:MaxGCPauseMillis"}, infoI18n = {"每次 GC 最大的停顿毫秒数，VM 将调整 Java 堆大小和其他与 GC 相关的参数，以使 GC 引起的暂停时间短于该毫秒，尽可能地保证内存回收花费时间不超过设定值（-XX:MaxGCPauseMillis）。", "en:For each GC maximum pause milliseconds, the VM will adjust the Java heap size and other GC-related parameters so that the pause caused by GC is shorter than that millisecond, ensuring that memory reclamation takes no longer than the set value as much as possible (-XX:MaxGCPauseMillis)."})
    @FieldView(group = group_gc, type = FieldType.number)
    @FieldValidation(effectiveWhen = "useGC=UseParallelGC", numberMin = 1, numberMax = 100)
    public Integer MaxGCPauseMillis;
    @ModelField(nameI18n = {"回收时间占比", "en:GCTimeRatio"}, infoI18n = {"设置垃圾回收时间占程序运行时间的百分比 (-XX:GCTimeRatio)。", "en:Sets the garbage collection time as a percentage of the program running time (-XX:GCTimeRatio)."})
    @FieldView(group = group_gc, type = FieldType.number)
    @FieldValidation(effectiveWhen = "useGC=UseParallelGC", numberMin = 1, numberMax = 100)
    public Integer GCTimeRatio;

    @ModelField(nameI18n = {"记录 GC 日志", "en:Print GC "}, infoI18n = {"设置是否记录 JVM 的 GC 日志。", "en:Sets whether to record GC logs for the JVM."})
    @FieldView(group = group_gc, type = FieldType.bool)
    public boolean gcLogEnabled;
    @ModelField(nameI18n = {"记录细节", "en:GC Details"}, infoI18n = {"设置是否记录 GC 的详细信息 (-XX:+PrintGCDetails)。", "en:Set whether GC details are logged (-XX:+PrintGCDetails)."})
    @FieldView(group = group_gc, type = FieldType.bool)
    @FieldValidation(effectiveWhen = "gcLogEnabled=true")
    public boolean PrintGCDetails;
    @ModelField(nameI18n = {"记录系统停顿时间", "en:Application StoppedTime"}, infoI18n = {"是否在 GC 日志中记录系统停顿时间。仅适用于 java8 (-XX:+PrintGCApplicationStoppedTime)。", "en:Whether application StoppedTime are recorded in GC logs, only available for java8 (-XX:+PrintGCApplicationStoppedTime)."})
    @FieldView(group = group_gc, type = FieldType.bool)
    @FieldValidation(effectiveWhen = "gcLogEnabled=true")
    public boolean PrintGCApplicationStoppedTime;
    @ModelField(nameI18n = {"记录系统执行时间", "en:Application ConcurrentTime"}, infoI18n = {"是否在 GC 日志中记录系统执行时间。仅适用于 java8 (-XX:+PrintGCApplicationConcurrentTime)。", "en:Whether application ConcurrentTime are recorded in GC logs, only available for java8 (-XX:+PrintGCApplicationConcurrentTime)."})
    @FieldView(group = group_gc, type = FieldType.bool)
    @FieldValidation(effectiveWhen = "gcLogEnabled=true")
    public boolean PrintGCApplicationConcurrentTime;
    @ModelField(nameI18n = {"记录堆信息", "en:Print Heap"}, infoI18n = {"是否在 GC 日志中记录堆信息，仅适用于java8 (-XX:+PrintHeapAtGC)。", "en:Whether heap information is recorded in the GC log, only available for java8 (-XX:+PrintHeapAtGC)."})
    @FieldView(group = group_gc, type = FieldType.bool)
    @FieldValidation(effectiveWhen = "gcLogEnabled=true")
    public boolean PrintHeapAtGC;
    @ModelField(nameI18n = {"GC 日志文件", "en:GC Log Path"},
    ...
    @FieldValidation(effectiveWhen = "gcLogEnabled=true", required = true)
    @FieldView(group = group_gc)
    public String gcLog = "logs/gc/gc.log";

    @ModelField(nameI18n = {"开启堆转储", "en:Heap Dump"}, infoI18n = {"当 JVM 发生 OOM 时，自动生成 DUMP 文件，文件位置由【堆转储文件】选项指定 (-XX:+HeapDumpOnOutOfMemoryError)。", "en:When OOM occurs in JVM, a dump file is automatically generated, and the file location is specified by the [Heap Dump Path] option (-XX:+HeapDumpOnOutOfMemoryError)."})
    @FieldView(group = group_heapDump, type = FieldType.bool)
    ...
    @FieldValidation(effectiveWhen = "HeapDumpOnOutOfMemoryError=true", required = true)
    @FieldView(group = group_heapDump)
    public String HeapDumpPath = "logs/heap_${TW_TimeStamp}.hprof";

    @ModelField(nameI18n = {"记录 JVM 日志", "en:Log VM Output"}, infoI18n = {"设置是否记录 JVM 日志 (-XX:+LogVMOutput)。", "en:Set whether to log JVM logs (-XX:+LogVMOutput)."})
    @FieldView(group = group_jvmLog, type = FieldType.bool)
    ...
    @FieldValidation(effectiveWhen = "LogVMOutput=true", required = true)
    @FieldView(group = group_jvmLog)
    ...
    @FieldView(group = group_environment)
    public String JAVA_HOME;
    @ModelField(nameI18n = {"环境变量", "en:Environments"},
            infoI18n = {"设置应用程序所需要的环境变量。", "en:Set the environment variables required by the application."})
    @FieldView(group = group_environment, type = FieldType.kv)
    public String envs;
    ...
    @FieldView(group = group_IP, type = FieldType.bool)
