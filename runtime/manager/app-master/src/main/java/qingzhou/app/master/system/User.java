...
public class User extends ModelBase implements Createable {
    ...
    @ModelField(
            group = "security",
            type = FieldType.bool, shownOnList = true, nameI18n = {"是否激活", "en:Is Active"}, infoI18n = {"若未激活，则无法登录服务器。", "en:If it is not activated, you cannot log in to the server."})
    public Boolean active = true;

    @ModelField(
            group = "security",
            shownOnList = true,
            cannotAdd = true, cannotUpdate = true,
    ...
    @ModelField(
            group = "security",
            type = FieldType.number, numberMin = 1, numberMax = 10,
