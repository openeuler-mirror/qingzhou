...
public class Launcher {
    ...
    static void checkLatestZipVersion() throws Exception {
        String zipLatestVer = "";
        String format = ".zip";
        File[] listFiles = new File(homeFile, "lib").listFiles(f -> (f.getName().startsWith("version") && f.getName().endsWith(format)));
        if (listFiles != null) {
            for (File file : listFiles) {
                String fileName = file.getName();
                String zipVer = fileName.substring("version".length(), fileName.length() - format.length());
                if (isLaterVersion(zipVer, zipLatestVer)) {
                    zipLatestVer = zipVer;
                }
            }
        }

        File verFile = Paths.get(homeFile.getAbsolutePath(), "lib", "version" + zipLatestVer + format).toFile();
        if (verFile.exists()) {
    ...
    static File getLatestLibDir() {
        ...
        String version = "";
        File[] listFiles = new File(homeFile, "lib").listFiles(f -> f.isDirectory() && f.getName().startsWith("version"));
        if (listFiles != null) {
            for (File file : listFiles) {
