			...
			<tr style="height:20px;">
				...
				<%
					...
					if (modelInfo.isShowOrderNumber()) {
						otherTh += 1;
				%>
				<th class="sequence"><%=I18n.getKeyI18n("page.list.order")%></th>
				<%
					}
				...
				<th style="width: <%=width%>%"><%=I18n.getModelI18n(qzApp, "model.field." + qzModel + "." + field)%></th>
				<%
					}
					if (listActions.length > 0) {
		...
		<tr>
			...
			<td>
				...
				<a href='<%=PageUtil.buildCustomUrl(request, response, qzRequest,HtmlView.FLAG, refModelName, qingzhou.api.type.List .ACTION_LIST + "?" + refFieldName + "=" + refValue)%>'
				   class="dataid qz-action-link tooltips" data-tip='<%=I18n.getModelI18n(qzApp, "model." + refModelName)%>' data-tip-arrow="top" 
            ...
            <td>
				...
				<a href="<%=PageUtil.buildRequestUrl(request, response, qzRequest, useJsonUri ? JsonView.FLAG : HtmlView.FLAG, actionName + "/" + encodedItemId)%>"
					data-tip='<%=I18n.getModelI18n(qzApp, "model.action.info." + qzModel + "." + actionName)%>' data-tip-arrow="top"
					class="qz-action-link tooltips" model-icon="<%=modelInfo.getIcon()%>"
				    data-id="<%=(qzModel + "|" + encodedItemId)%>" data-action="<%=action.getActionType()%>" data-name="<%=originUnEncodedId%>"
						<%
