...
import qingzhou.engine.util.jnet.Installer;
import qingzhou.engine.util.jnet.URLStreamHandlerFactoryWrapper;
...
public class TomcatURLStreamHandlerFactory implements URLStreamHandlerFactory {
    ...
    private TomcatURLStreamHandlerFactory(boolean register) {
        ...
        if (register) {
            // URL.setURLStreamHandlerFactory(this); // by wangpl, for: java.lang.Error: factory already defined

            try { // added by wangpl: felix 已经设置了一次，tomcat无法继续设置，所以采用此方式
                Installer.setURLStreamHandlerFactory(new URLStreamHandlerFactoryWrapper(this));
            } catch (Exception e) {
