    ...
    <body class="login_body" onload="escapeFrame();">
        ...
        <main class="page page-login text-center">
            ...
            <section class="panel-body">
                <div class="logo"><%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "page.userlogin")%></div>
                <form id="loginForm" method="post" action="<%=ConsoleUtil.encodeURL(request, response, contextPath+LoginManager.LOGIN_URI)%>" class="form-group" autocomplete="off">
                    <div class="input-control has-icon-left">
                        <input value="thanos" type="text" id="<%=LoginManager.LOGIN_USER%>" name="<%=LoginManager.LOGIN_USER%>" class="form-control" placeholder="name<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "model.field.user.name")%>" autofocus required>
                    ...
                    <div class="input-control has-icon-left">
                        <input value="thanos123.com" type="text" id="<%=LoginManager.LOGIN_PASSWORD%>_txt" data-type="password" class="form-control" placeholder="password<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "model.field.user.password")%>" dotted onchange="document.getElementById('<%=LoginManager.LOGIN_PASSWORD%>').value = this.value;">
                    ...
                    <div class="input-control has-icon-left">
                        <input type="text" id="<%=ConsoleConstants.LOGIN_2FA%>_txt" class="form-control" placeholder="<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "page.info.2fa")%>" onchange="document.getElementById('<%=ConsoleConstants.LOGIN_2FA%>').value = this.value;">
                    ...
                    %>
                    <div class="input-control has-icon-left">
                        <input type="text" id="<%=VerCode.CAPTCHA%>" name="<%=VerCode.CAPTCHA%>" class="form-control" required style="width:250px;display:inline-block;float:left;" placeholder="<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "page.vercode")%>">
                    ...
                    <input type="submit" value='<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "page.login")%>' class="login_btn">
...
for (Lang lang : Lang.values()) {
    %>
    <h3 align="center"><%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "AGREEMENT_HEADER", lang)%></h3>
    <%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "AGREEMENT_BODY", lang)%>
    <br><br>
    <p align="center">----------------------------------------------------------------------------</p>
    <%
}
%>
        ...
        <footer class="page-copyright">
            <%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, "page.copyright")%>&nbsp;<a href="https://www.openeuler.org/" target="_blank">www.openeuler.org</a>
        </footer>
        ...
        <script type="text/javascript">
            ...
            $("#loginForm").submit(function (e) {
                if ($("#<%=LoginManager.LOGIN_ACCEPT_AGREEMENT%>").is(":checked")) {
                    ...
                    layer.msg("<%=I18n.getString(ConsoleConstants.MASTER_APP_NAME, LoginManager.ACCEPT_AGREEMENT_MSG_KEY)%>", function () {});
                    return false;
                }
            });
