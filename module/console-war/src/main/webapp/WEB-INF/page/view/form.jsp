...
<%
    ...
    String submitActionName = ConsoleWarHelper.getSubmitActionName(qzRequest);
...
<form name="pageForm"
    ...
    <div class="block-bg" style="padding-top: 24px; padding-bottom: 1px;">
        ...
        if (!models.isEmpty()) {
            model = models.get(0);
            Map<String, Map<String, ModelField>> fieldMapWithGroup = ConsoleWarHelper.getGroupedModelFieldMap(qzRequest);
            ...
            for (String group : groups) {
                ...
                for (Map.Entry<String, ModelField> e : groupFieldMap.entrySet()) {
                    ...
                    if (ConsoleWarHelper.isFieldReadOnly(qzRequest, fieldName)) {
                        readonly = "readonly";
                    }
                            ...
                            if (StringUtil.notBlank(readonly)) {
                                ...
                                switch (modelField.type()) {
                                    ...
                                    case password:
                                        passwordFields.add(fieldName);
                                        %>
                                        <input type="password" name="<%=fieldName%>" value='<%=fieldValue%>' data-type="password" class="form-control">
                                        <label password_label_right="<%=fieldName%>_eye" class="input-control-icon-right" style="margin-right: 10px; cursor: pointer;"><i class="icon icon-eye-close"></i></label>
                                        <%
                                        break;
                                    case textarea:
                                        %>
    ...
    <textarea name="eventConditions" rows="3" disabled="disabled" style="display:none;">
        ...
        Map<String, String> conditions = ConsoleWarHelper.modelFieldEffectiveWhenMap(qzRequest);
