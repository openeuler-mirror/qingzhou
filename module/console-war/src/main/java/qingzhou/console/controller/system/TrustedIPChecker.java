...
public class TrustedIPChecker implements Filter<HttpServletContext> {
    ...
    public boolean doFilter(HttpServletContext context) throws Exception {
        ...
        String toJson = JsonView.buildErrorResponse(I18n.getString(qingzhou.framework.api.Constants.MASTER_APP_NAME, msgKey));
        if (I18n.getI18nLang() == Lang.en) { // header里只能英文
            response.setHeader(qingzhou.framework.api.Constants.RESPONSE_HEADER_MSG_KEY, toJson);// 重定向，会丢失body里的消息，所以用header
        } else {
