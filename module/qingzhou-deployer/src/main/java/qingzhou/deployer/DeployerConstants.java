...
public interface DeployerConstants {
    ...
    String DASHBOARD_FIELD_BASIC_DATA = DashboardDataBuilder.Basic.class.getSimpleName();
    // 定义仪表盘图表的仪表盘数据标识，前端返回值使用
    String DASHBOARD_FIELD_GAUGE_DATA = DashboardDataBuilder.Gauge.class.getSimpleName();
    // 定义仪表盘图表的直方图数据标识，前端返回值使用
    String DASHBOARD_FIELD_HISTOGRAM_DATA = DashboardDataBuilder.Histogram.class.getSimpleName();
    // 定义仪表盘图表的共享数据集数据标识，前端返回值使用
    String DASHBOARD_FIELD_SHARE_DATASET_DATA = DashboardDataBuilder.ShareDataset.class.getSimpleName();
}
