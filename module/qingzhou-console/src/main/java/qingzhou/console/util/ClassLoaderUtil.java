...
public class ClassLoaderUtil {
    ...
    // NOTE: URLClassLoader使用完毕要及时 close 掉
    public static URLClassLoader newExtLoader(File file, ClassLoader parentLoader) {
        if (file.isDirectory()) {
            File[] listFiles = file.listFiles();
            return newURLClassLoader(listFiles, parentLoader);
        } else {
            return newURLClassLoader(file, parentLoader);
        }
    }
