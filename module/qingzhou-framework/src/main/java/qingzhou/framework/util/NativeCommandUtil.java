...
public class NativeCommandUtil {
    ...
    public static int runNativeCommand(String command, File dir, StringCollector output) throws Exception {
        return runNativeCommand(command, dir, output, 15);
    }

    public static int runNativeCommand(String command, File dir, StringCollector output, int waitSeconds) throws Exception {
        return runNativeCommand(command, dir, null, output, waitSeconds);
    }

    public static int runNativeCommand(String command, File dir, String[] envp, StringCollector output, int waitSeconds) throws Exception {
        if (output != null) {
            return runNativeCommand0(command, dir, envp, output, waitSeconds);
        } else {
            StringCollector collectorUtil = new StringCollector();
            try {
    ...
    private static int runNativeCommand0(String command, File dir, String[] envp, StringCollector output, int waitSeconds) throws Exception {
    ...
    public static int forcedStop(String pid, StringCollector output) throws Exception {
